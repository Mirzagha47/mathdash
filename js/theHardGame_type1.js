...
theHardGame.prototype = {
  	create: function(){
		...
		scorePatch1 = this.game.add.sprite(this.xPostion(50),  this.yPostion(55), "scorePatch1");
		scorePatch1.anchor.set(0.5)
		circle1 = new PieProgress(this.game, this.xPostion(50),  this.yPostion(55), 360,"#ffcc00");
		//circle1.alpha = 0;
		...
		answerTxt = this.game.add.bitmapText(this.game.width/2,  this.game.height * 0.565, 'number', '1', 140);
	  ...
	  scoreLabel = this.game.add.bitmapText(this.game.width * 0.25 , this.game.height * 0.075, 'question', 'Score : 0', 70);
    ...
    //Create a tween to grow / shrink the score label
     scoreLabelTween = this.add.tween(scoreLabel.scale).to({ x: 1.5, y: 1.5}, 200, Phaser.Easing.Linear.In).to({ x: 1, y: 1}, 200, Phaser.Easing.Linear.In);
		 ...
		 if(AdMob)
		AdMob.prepareInterstitial( {adId:"ca-app-pub-9157208114664226/5357086598", autoShow:false} );
	...
	{
		var firstCount = localStorage.getItem("isFirstTime");
		if(firstCount == null)
		{
	...
	showPopUp:function()
	{
		...
		currentOperator = "+";
		isHandAdded = true;
		introPopUp = this.game.add.button(this.xPostion(2), this.yPostion(40),'firstHint',this.popupClose,this);
	},
	popupClose:function()
	{
		introPopUp.destroy();
	...
	onComplete:function()
	{
			...
			if(isAdReady && isAdUnlcoked == "true")
			{
				if(AdMob)
	...
	generateQuestion:function()
	{
		...
		var no1 = this.randomRange(-10,10);
		var no2 = this.randomRange(-10,10);
		...
		else
		{
			...
			else if(scoreBuffer < 10)
			{
				speed = 4000;
			}
			else if(scoreBuffer < 20)
			{
				speed = 3000;
			}
			else
